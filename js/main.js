...
document.addEventListener("DOMContentLoaded", function () {
  ...
  //   1. Mint the reference HERE, in the browser.
  //   2. POST the form to Code.gs with that reference included. Code.gs writes
  //      the row using the reference it was handed.
  //   3. Send the browser to the Stripe Payment Link carrying the same
  //      reference as client_reference_id.
  //   4. Stripe's webhook posts back to StripeWebhookCode.gs, which finds the
  //      row with that reference and sets Payment status to Paid.
  //
  // Why the reference is minted client-side (changed 9/25/26):
  // It used to be generated by Code.gs and read back out of the response. That
  // read is the one step that kept failing. Apps Script answers a POST with a
  // 302 to script.googleusercontent.com, and that follow-up request was
  // returning 404 - so the script ran fine and wrote the row every time (the
  // execution log shows doPost Completed on every attempt), but the browser
  // could not retrieve the reply, saw a non-ok response, and showed the error
  // instead of going to Stripe. Nobody could pay.
  //
  // Holding the reference before we send it removes that dependency entirely:
  // there is nothing to read back, so a flaky response cannot strand anyone.
  //
  // The sheet write is still kicked off first on purpose: if someone abandons
  // the Stripe page we still have their details and can follow up.
  //
  ...
  // HC- plus 8 chars. Alphabet omits I, L, O, 0 and 1 so a reference read off a
  // screen or over the phone cannot be transcribed two different ways. Matches
  // the shape makeReference() produces in Code.gs.
  function hcMakeRetreatReference() {
...
(function () {
  ...
  form.addEventListener("submit", async function (e) {
    ...
    // Mint it before sending so both the sheet row and the Stripe handoff carry
    // the same value. Code.gs writes whatever it is given here.
    const reference = hcMakeRetreatReference();

    const payload = new FormData(form);
    payload.set("reference", reference);

    // Build the Stripe URL now, while the field values are still in hand.
    const stripeUrl = new URL(HC_RETREAT_PAYMENT_LINK);
    stripeUrl.searchParams.set("client_reference_id", reference);
    if (email) stripeUrl.searchParams.set("prefilled_email", email);

    try {
      // keepalive lets the request finish even though we navigate away from
      // this page a moment later.
      ...
      // Deliberately swallowed. A throw here means we could not read Apps
      // Script's reply - it does NOT mean the row failed to write, and in
      // practice the write has succeeded every time while the reply did not
      // come back. Blocking the redirect on this is what left people unable to
      // pay, so we log it and carry on to Stripe.
      console.warn("[retreat] response not readable, continuing:", err);
    }
